...
Stated_Seg <- data.frame(Year=2019, Precipitation=517, Station="Stated_Seg")
Stated_Ses <- data.frame(Year=2019, Precipitation=458, Station="Stated_Ses")
...
(AnnualPrecip_plots <- ggplot(AnnualPrecip, aes(x=Year, y=Precipitation, group=Station)) +
    ...
    geom_line(aes(y = mean_precip_ID40)) +
    geom_line(aes(y = (mean_precip_ID49-(iqr_ID40/2))), linetype = "dotted") +
    geom_line(aes(y = (mean_precip_ID49+(iqr_ID40/2))), linetype = "dotted") +
